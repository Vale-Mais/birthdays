"""This module takes all the functions
from the package and gives the possibility
to the user to interact with the database
containing the names and the birthdays of
known people """
...
if args.add:
    ...
    print("The birthday of ", answer, "is ", db["Birthday"].loc[
           db["Person"].str.lower() == answer.lower()].values[0])
else:
    response = input(answer + " is not present in our database." +
                     "Would you like to add him? (yes or no) -> ")
    if response == "yes":
        add_person(answer)
    else:
        print("No worries!")
